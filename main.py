...
def generate_data(student_number, n_samples=1000):
    ...
    df = pd.DataFrame(data, columns=['Płeć', 'Wiek', 'Wykształcenie', 'Średnie Zarobki', 'Czas Początkowy Podróży',
                                     'Czas Końcowy Podróży', 'Cel Podróży'])
    ...
    for _ in range(n_invalid):
        ...
        if random.random() < 0.5:
            ...
            df.at[
                idx, 'Czas Końcowy Podróży'] = f"{(int(df.at[idx, 'Czas Początkowy Podróży'].split(':')[0]) + 13) % 24:02}:{random.randint(0, 59):02}"
...
if __name__ == "__main__":
    ...
    #Jest to do testowania gdyż aktualnie działa GitHub Actions, gdyż brak funduszy
